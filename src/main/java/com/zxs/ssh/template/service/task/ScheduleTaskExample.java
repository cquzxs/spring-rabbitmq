...
import com.zxs.ssh.template.service.rabbitmq.producer.LogProducer;
...
public class ScheduleTaskExample {
    ...
    @Resource(name = "logProducer")
    private LogProducer logProducer;

    @Scheduled(cron = "0 * * ? * *")    //9点的每分钟执行一次  cron表达式： 秒 分钟 小时 日期 月份 星期
    public void timeTask(){
        logProducer.sendError("error");
        logProducer.sendWarn("warn");
        logProducer.sendInfo("info");
    }
}
